...
import { ImageItem } from "@/types/gpt.type";

const { loading } = storeToRefs(useSystemStore());

const { messageList } = defineProps({
  messageList: Array<ImageItem | MessageItem>,
});
...
<template>
  <div class="message-box">
    ...
    >
      ...
      >
        <div v-if="item.role != 'image'">
          {{ item.content }}
        </div>
        <div v-else>
          <v-img
            v-for="(image, index) in (item.content as Array<{url:string}>)"
            :key="index"
            :width="200"
            :height="200"
            aspect-ratio="1/1"
